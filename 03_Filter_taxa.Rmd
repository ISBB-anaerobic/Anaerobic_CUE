...
Modelling library size shows a significant effect of read depth on the community structure, but explaining only `r percent(mod1$aov.tab$R2[4])` of the variance.
...
Removed `r percent(Summary_pruned$Seqs.removed[Summary_pruned$Level == "Order"] / sum(sample_sums(Ps_obj_SIP_merged)), accuracy = 0.0001)` of the sequences.
...
This removed `r ntaxa(Ps_obj_SIP_merged_filt) - ntaxa(Ps_obj_SIP_merged_filt2)` or `r percent(1 - (ntaxa(Ps_obj_SIP_merged_filt2) /  ntaxa(Ps_obj_SIP_merged_filt)))` of the ESVs, and `r percent(1 - (sum(otu_table(Ps_obj_SIP_merged_filt2)) /  sum(otu_table(Ps_obj_SIP_merged_filt))), accuracy = 0.001)` of the reads.
