...
options(width = 90, knitr.table.format = "html", getClass.msg=FALSE) 
...
prep_DESeq_data <- function(DESeq_results, ps_obj, sig_level = alpha_thresh, LFC = LFC_thresh, rank = "Phylum", rare_thresh = 0.01) {
  ...
  # mark rare phyla
  # We're doing this now and not after removing 0-abundance ASVs because we want the order to be consistent over all the DESeq plots we create from the dataset
  ...
  DESeq_results[[1]] %>% 
    ...
    bind_cols(., as(tax_table(ps_obj)[taxa_names(ps_obj) %in% .$OTU, ], "data.frame")) %>% # add taxonomy
...
plot_otus_by_density <- function(ps_obj = Ps_obj_SIP_noTime_l[[1]], 
  ...
  ggplot(Incorporators_df, aes(x = !!sym(X_val),
    ...
    labs(x = "Density (g ml<sup>-1</sup>)",
         y = "Abundance (%)",
         title = paste(ASV2plot$Site[1], "&", ASV2plot$Oxygen[1])) +
...
labels <- setNames(glue("<i style='colors:{colors}'>{labels2draw}</i>"), labels2draw)
  ...
  DESeq_res_df_sub %>% 
    ...
    mutate(across(ASV, ~factor(., levels = tip_order))) %>% # sort ASV levels according to the tree tip
    ...
    guides(fill = guide_legend(title = "Log<sub>2</sub> fold change",
                               title.position = "top")) +
    facet_wrap(vars(!!sym("Site_Oxygen")), 
...
sum_labelled_ASVs <- function(Ps_obj_SIP, DESeq_res_SIP_byTime_LFC_sig_df, site = "Čertovo", oxygen = "Oxic", Time = "24 h") {
  site <<- site # this is needed because of the scoping issue of subset_samples()
  oxygen <<- oxygen
  nTime <<- as.numeric(str_remove(Time, " h"))
  Ps_obj_SIP %>% 
  transform_sample_counts(function(x) {x/sum(x)}) %>% 
  prune_taxa(filter(DESeq_res_SIP_byTime_LFC_sig_df, Site == site & Oxygen == oxygen & Hours == Time)$ASV, .) %>% 
  subset_samples(Site == site & Oxygen == oxygen) %>%
  subset_samples(Hours == nTime) %>%
  otu_table() %>% 
  as(., "matrix") %>% 
  rowSums() %>% 
  data.frame(Total = .) %>% 
  ...
  group_by(Site, Oxygen, Hours) %>% 
  summarise_if(is.numeric, mean) ->
  summed_ASVs
  return(summed_ASVs)
}

sum_base_means <- function(ps_obj = Ps_obj_SIP, drop_nulls = TRUE){
  ps_obj %>%
    transform_sample_counts(., function(x) x / sum(x) * 100) %>% 
    taxa_sums(.) %>%
    {if(drop_nulls) .[.!=0] else .} %>% # remove 0 
    map_dbl(~(.x / nsamples(Ps_obj_SIP))) %>% 
...
```
```{r plot NTI, fig.height=5, fig.width=6, cache=T}
