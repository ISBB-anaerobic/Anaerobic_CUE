...
if (length(args) == 0) {
  ...
  paste("Pooling, data path and db path args supplied")
  (pooling <- args[1])
  (data_path <- args[2])
  (db_path <- args[3])
} else if (length(args) == 4) {
  paste("Pooling, data path, db path and mock id args supplied")
  (pooling <- args[1])
  (data_path <- args[2])
  (db_path <- args[3])
  (mock_id <- args[4])
...
# list.files(db_path)
silva_db <- "SILVA/silva_nr_v132_train_set.fa.gz"
silva_sp_db <- "SILVA/silva_species_assignment_v132.fa.gz"
gtdb_db <- "GTDB/GTDB_bac-arc_ssu_r86.fa.gz"
...
print("First 10 merged sequences:")
head(mergers[[1]], n = 10)
...
print("Remove sequences differing from excected lenght by more than 5 bp")
...
print("Sequence table dimensions after chimera removal:")
dim(seqtab_nochim)
print("What proportions passed through chimera check?")
...
} else {paste("No mock community identifier supplied")}
